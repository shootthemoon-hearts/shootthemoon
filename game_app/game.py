...
class Game():
    ...
    def addPlayer(self, channel):
        ...
        game_event_manager.multiplex_transmit(channel,{'id':str(self.gameID)})
        new_player = Player(channel)
        self.players.append(new_player)
        position = len(self.players) - 1
        new_player.position = position
        game_event_manager.multiplex_transmit(channel,{"player_pos":new_player.position})
    ...
    def send_players_their_cards(self):
        ...
        for player in self.players:
            ...
            game_event_manager.multiplex_transmit(player.channel,{"Cards":cards_str})
            
    def send_player_valid_cards(self, channel, valid_cards):
        cards_str = ""
        for card in valid_cards:
                cards_str += card.to_json()
        game_event_manager.multiplex_transmit(channel,{"valid_cards":cards_str})
    ...
    def send_players_the_phase(self, phase):
        ...
        theirs'''
        for player in self.players:
            game_event_manager.multiplex_transmit(player.channel,{"game_phase": phase})
    ...
    def send_players_discard(self, player, discard):
        ...
        game_event_manager.multiplex_transmit(self.group,{"discard": {"player": player.position, "card": discard_json}})
    ...
    def send_players_score(self):
        ...
        game_event_manager.multiplex_transmit(self.group,{"scores": {"player": player.position, "score_list": score_list}})
